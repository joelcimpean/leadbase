...
Website findings: ${JSON.stringify(lead.website_findings ?? null).slice(0, 8000)}
...
Create a conservative proposal starting draft that the freelancer can review later.
    ...
    const branding = metadata.leadbase_proposal_branding && typeof metadata.leadbase_proposal_branding === "object"
      ? metadata.leadbase_proposal_branding as Record<string, unknown>
      : {};

    const accentColor = typeof branding.accentColor === "string" && /^#[0-9a-f]{6}$/i.test(branding.accentColor)
      ? branding.accentColor.toUpperCase()
      : "#002BBA";
    const logoUrl = typeof branding.logoUrl === "string" ? branding.logoUrl : null;
    const logoPath = typeof branding.logoPath === "string" ? branding.logoPath : null;
