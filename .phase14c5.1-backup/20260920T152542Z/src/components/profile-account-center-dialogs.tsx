...
export function ManagePlanDialog({
  ...
  const [billingCurrency, setBillingCurrency] = useState<LeadbaseBillingCurrency>(() =>
    normalizeBillingCurrency(initial.billingCurrency, initialBillingCurrency)
  );
  ...
  const selectedIsCurrent =
    ...
    (!initial.billingCurrency || billingCurrency === initial.billingCurrency);

  async function chooseBillingCurrency(value: string) {
    const next = normalizeBillingCurrency(value, initialBillingCurrency);
    setBillingCurrency(next);
    onBillingCurrencyChange(next);
    const result = await saveBillingCurrencyPreference(next);
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4 sm:px-6">
        <div className="flex flex-wrap items-end gap-4 rounded-[12px] bg-[#F7F8FA] p-3.5 dark:bg-white/[.055]">
          <div>
            ...
            <div className="mt-1.5 flex items-center gap-2">
              ...
              <span className="rounded-[6px] bg-[#EAEEFB] px-2 py-0.5 font-mono text-[9px] uppercase tracking-[.06em] text-[#002BBA] dark:bg-[#002BBA]/20 dark:text-[#8EA5FF]">{billingCurrency === "USD" ? "$" : ""}<AnimatedNumber value={price} locale={billingCurrency === "USD" ? "en-US" : "de-DE"} />{billingCurrency === "EUR" ? " €" : ""} / {billing === "monthly" ? (language === "de" ? "Monat" : "month") : (language === "de" ? "Jahr" : "year")}</span>
            </div>
          </div>
        ...
        <div className="mt-4 flex w-fit rounded-[10px] bg-[#F3F4F6] p-0.5 dark:bg-white/[.07]">
          ...
          <select
            ...
            onChange={(event) => void chooseBillingCurrency(event.target.value)}
            aria-label={language === "de" ? "Abrechnungswährung" : "Billing currency"}
            className="h-9 rounded-[9px] border border-black/10 bg-white px-3 font-mono text-[10.5px] font-medium text-[#40454E] outline-none transition focus:border-[#002BBA] dark:border-white/15 dark:bg-[#121316] dark:text-[#E8E8EA]"
          >
            ...
            return (
              <button key={plan.id} type="button" onClick={() => setPlanId(plan.id)} className={cn("rounded-[12px] border bg-white p-3.5 text-left transition dark:bg-[#121316]", active ? "border-[#002BBA] shadow-[0_0_0_3px_rgba(0,43,186,.10)]" : "border-black/10 hover:border-[#002BBA]/45 dark:border-white/12")}>
                ...
                <div className="mt-3 flex items-baseline gap-1.5">
                  <strong className="text-[18px] font-semibold tracking-[-.025em] tabular-nums dark:text-white">{billingCurrency === "USD" ? "$" : ""}<AnimatedNumber value={tierPrice} locale={billingCurrency === "USD" ? "en-US" : "de-DE"} />{billingCurrency === "EUR" ? " €" : ""}</strong>
...
export function BuyCreditsDialog({
  ...
  const [topupCurrency, setTopupCurrency] = useState<LeadbaseBillingCurrency>(() =>
    normalizeBillingCurrency(initialBillingCurrency, "USD")
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  ...
  async function chooseTopupCurrency(value: string) {
    const next = normalizeBillingCurrency(value, "USD");
  ...
  return (
    ...
    >
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4 sm:px-6">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-[10px] border border-black/[.07] bg-[#FAFBFC] px-3 py-2 dark:border-white/10 dark:bg-white/[.035]">
          <div>
            <span className={monoClass}>{language === "de" ? "Abrechnungswährung" : "Billing currency"}</span>
            <p className="mt-0.5 text-[9.5px] text-[#7A7F88] dark:text-[#91959D]">{language === "de" ? "Unabhängig von deiner Workspace-Währung · wird für den nächsten Kauf gemerkt." : "Separate from workspace currency · remembered for your next purchase."}</p>
          </div>
          <select
            value={topupCurrency}
            onChange={(event) => void chooseTopupCurrency(event.target.value)}
            aria-label={language === "de" ? "Credit-Abrechnungswährung" : "Credit billing currency"}
            className="h-9 rounded-[9px] border border-black/10 bg-white px-3 font-mono text-[10.5px] font-medium text-[#40454E] outline-none focus:border-[#002BBA] dark:border-white/15 dark:bg-[#121316] dark:text-[#E8E8EA]"
          >
            {LEADBASE_BILLING_CURRENCIES.map((code) => <option key={code} value={code}>{code}</option>)}
          </select>
        </div>
          ...
          {LEADBASE_CREDIT_TOPUPS.map((pack) => {
            ...
            return (
              <button key={pack.id} type="button" onClick={() => setSelected(pack.id)} className={cn("rounded-[12px] border bg-white p-3.5 text-left transition dark:bg-[#121316]", active ? "border-[#002BBA] shadow-[0_0_0_3px_rgba(0,43,186,.10)]" : "border-black/10 hover:border-[#002BBA]/45 dark:border-white/12")}>
                ...
                <div className="mt-3 border-t border-black/[.07] pt-2.5 dark:border-white/10"><strong className="text-[13px] dark:text-white">{topupCurrency === "USD" ? "$" : ""}{priceForTopup(pack, topupCurrency)}{topupCurrency === "EUR" ? " €" : ""}</strong></div>
              </button>
            );
          })}
        </div>
        ...
        >
          <button type="button" onClick={() => setSelected("custom")} className="flex w-full items-center gap-3 text-left">
            ...
            <div className="text-right">
              ...
              <strong className="mt-1 block text-[16px] font-semibold tabular-nums dark:text-white">{topupCurrency === "USD" ? "$" : ""}<AnimatedNumber value={customCreditPrice(customCredits, topupCurrency) ?? 0} locale={topupCurrency === "USD" ? "en-US" : "de-DE"} />{topupCurrency === "EUR" ? " €" : ""}</strong>
            </div>
      ...
      <div className="flex shrink-0 items-center justify-end gap-2 border-t border-black/[.07] px-5 py-3 sm:px-6 dark:border-white/10">
        <button type="button" onClick={onClose} className="h-9 rounded-[9px] border border-black/10 px-3.5 text-[12.5px] font-medium text-[#40454E] dark:border-white/15 dark:text-[#E8E8EA]">{language === "de" ? "Abbrechen" : "Cancel"}</button>
        <button type="button" onClick={() => void checkout()} disabled={saving} className="flex h-9 items-center gap-2 rounded-[9px] bg-[#002BBA] px-3.5 text-[12.5px] font-medium text-white disabled:opacity-55">{saving ? <Loader2 className="size-3.5 animate-spin" /> : <Zap className="size-3.5" />}{language === "de" ? <>{selectedCredits.toLocaleString("de-DE")} Credits · {topupCurrency === "USD" ? "$" : ""}<AnimatedNumber value={selectedPrice} locale={topupCurrency === "USD" ? "en-US" : "de-DE"} />{topupCurrency === "EUR" ? " €" : ""}</> : <>{selectedCredits.toLocaleString("en-US")} credits · {topupCurrency === "USD" ? "$" : ""}<AnimatedNumber value={selectedPrice} locale={topupCurrency === "USD" ? "en-US" : "de-DE"} />{topupCurrency === "EUR" ? " €" : ""}</>}</button>
      </div>
    </DialogShell>
  );
}
