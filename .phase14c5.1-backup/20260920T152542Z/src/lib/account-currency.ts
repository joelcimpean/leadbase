...
export const LEADBASE_BILLING_CURRENCIES = ["USD", "EUR"] as const;
export type LeadbaseBillingCurrency = (typeof LEADBASE_BILLING_CURRENCIES)[number];
...
export function normalizeBillingCurrency(
  value: unknown,
  fallback: LeadbaseBillingCurrency = "USD",
): LeadbaseBillingCurrency {
  const upper = typeof value === "string" ? value.trim().toUpperCase() : "";
  return upper === "EUR" || upper === "USD" ? upper : fallback;
}

export function formatBillingMoney(value: number, currency: LeadbaseBillingCurrency) {
