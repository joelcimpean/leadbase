...
export function ManagePlanDialog({
  ...
  const [billingCurrency, setBillingCurrency] = useState<LeadbaseBillingCurrency>(() =>
    normalizeBillingCurrency(initial.billingCurrency, initial.priceEur > 0 ? "EUR" : "USD")
  );
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4 sm:px-6">
        ...
        <div className="mt-4 flex w-fit rounded-[10px] bg-[#F3F4F6] p-0.5 dark:bg-white/[.07]">
          ...
          <select
            ...
            onChange={(event) => setBillingCurrency(normalizeBillingCurrency(event.target.value))}
...
export function BuyCreditsDialog({
  ...
  const [topupCurrency, setTopupCurrency] = useState<LeadbaseBillingCurrency>("USD");
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
    ...
    >
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4 sm:px-6">
        <div className="mb-3 flex justify-end">
          <select
            value={topupCurrency}
            onChange={(event) => setTopupCurrency(normalizeBillingCurrency(event.target.value))}
