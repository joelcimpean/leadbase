...
function currentStepStartedAt(run: SafeRun | null) {
  if (!run?.steps || typeof run.steps !== "object") return null;
  const value = run.steps[run.current_step];
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
...
function workflowProgressCopy(step: string | null | undefined, de: boolean, elapsedSeconds: number) {
  const copy: Record<string, { en: string[]; de: string[] }> = {
    ...
    design: {
      en: ["Generating the design…", "Building the visual direction…", "Refining layout and hierarchy…", "Convincing the pixels to cooperate…"],
      de: ["Design wird erstellt…", "Visuelle Richtung wird aufgebaut…", "Layout und Hierarchie werden verfeinert…", "Die Pixel werden zur Mitarbeit überredet…"],
    },
    outreach: {
  ...
  const stage = copy[step || "queued"] ?? copy.queued;
  const variants = de ? stage.de : stage.en;
  return variants[Math.floor(elapsedSeconds / 6) % variants.length];
}
...
export function FullLeadWorkflowButton({
  ...
  const stageStartedAt =
    currentStepStartedAt(run) ??
    (run?.current_step === "finalizing" ? run.updated_at : null) ??
    run?.started_at ??
    null;
  const stageStartedMs = stageStartedAt ? new Date(stageStartedAt).getTime() : now;
  const elapsedSeconds = Number.isFinite(stageStartedMs)
    ? Math.max(0, (now - stageStartedMs) / 1000)
    : 0;
  const progressCopy = workflowProgressCopy(run?.current_step, de, elapsedSeconds);
  const progressTip = freeMode
    ? (de
      ? "Starter schaltet Evidence-basierte Website-Insights und weitere AI-Aktionen frei."
      : "Starter unlocks evidence-based website insights and continued AI actions.")
    : (de
      ? "Du kannst weiterarbeiten — der Fortschritt bleibt serverseitig gespeichert."
      : "You can keep working — progress stays stored on the server.");
        ...
        {(activeRun || starting) ? (
          <div className="mt-2 w-full max-w-[390px] rounded-[10px] border border-black/[0.07] bg-white px-3 py-2.5 text-[10.5px] shadow-sm dark:border-white/[0.08] dark:bg-[#121316]">
            <div className="mb-2 flex items-center justify-between gap-2">
              <span className="font-medium">{progressCopy}</span>
              <span className="shrink-0 font-mono text-[9px] font-semibold tabular-nums text-[#002BBA] dark:text-[#91A5FF]">
            ...
            <p className="mt-2 text-[9.5px] leading-4 text-[#6B7078] dark:text-[#A8ABB2]">
              {progressTip}
            </p>
          </div>
        ) : null}
