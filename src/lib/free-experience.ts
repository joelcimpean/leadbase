...
const FREE_DISCOVERY_CLAIM_TTL_MS = 15 * 60 * 1000;

export type FreeDiscoverySessionStatus = {
  isFree: boolean;
  available: boolean;
  used: boolean;
  inProgress: boolean;
...
type FreeDiscoveryStateRow = {
  free_discovery_claimed_at: string | null;
  free_discovery_claim_token: string | null;
  free_discovery_used_at: string | null;
  free_discovery_search_id: string | null;
  free_discovery_campaign_id: string | null;
};

export async function getFreeDiscoverySessionStatus(
  userId: string,
): Promise<FreeDiscoverySessionStatus> {
  const plan = await getLeadbasePlanAccess(userId);
  if (plan.planId !== "free") {
    return {
      isFree: false,
  ...
  const { data, error } = await admin
    ...
    .select(`
      ...
      free_discovery_search_id,
      free_discovery_campaign_id
    `)
    .eq("user_id", userId)
    .maybeSingle();

  if (error) {
    throw new Error(`FREE_DISCOVERY_STATE_UNAVAILABLE:${error.message}`);
  }

  const state = (data ?? null) as FreeDiscoveryStateRow | null;
  const used = Boolean(state?.free_discovery_used_at);
  const claimedAt = state?.free_discovery_claimed_at
    ? new Date(state.free_discovery_claimed_at).getTime()
    : 0;
  const inProgress =
    !used &&
    Boolean(state?.free_discovery_claim_token) &&
    Number.isFinite(claimedAt) &&
    claimedAt > Date.now() - FREE_DISCOVERY_CLAIM_TTL_MS;

  return {
    isFree: true,
    available: !used && !inProgress,
    used,
    inProgress,
    searchId: state?.free_discovery_search_id ?? null,
    campaignId: state?.free_discovery_campaign_id ?? null,
    usedAt: state?.free_discovery_used_at ?? null,
  };
}

export type FreeDiscoverySessionClaim = {
  token: string;
};

export async function claimFreeDiscoverySession(
  userId: string,
): Promise<FreeDiscoverySessionClaim | null> {
  const plan = await getLeadbasePlanAccess(userId);
  ...
  const { data, error } = await admin.rpc(
    "leadbase_claim_free_discovery_session",
    {
      p_user_id: userId,
      p_claim_token: token,
    },
  ...
  if (error) {
    const message = error.message || "FREE_DISCOVERY_CLAIM_FAILED";
    if (message.includes("FREE_DISCOVERY_ALREADY_USED")) {
      throw new Error("FREE_DISCOVERY_ALREADY_USED");
    }
    if (message.includes("FREE_DISCOVERY_IN_PROGRESS")) {
      throw new Error("FREE_DISCOVERY_IN_PROGRESS");
    }
    throw new Error(`FREE_DISCOVERY_CLAIM_FAILED:${message}`);
  }

  if (data !== token) {
    throw new Error("FREE_DISCOVERY_CLAIM_FAILED:token_mismatch");
  }

  return { token };
}

export async function finalizeFreeDiscoverySession(
  userId: string,
  claim: FreeDiscoverySessionClaim | null,
  input: {
    searchId: string;
    campaignId: string;
  },
  ...
  const admin = createAdminClient();
  const { data, error } = await admin.rpc(
    "leadbase_finalize_free_discovery_session",
    {
      p_user_id: userId,
  ...
  if (error) {
    throw new Error(`FREE_DISCOVERY_FINALIZE_FAILED:${error.message}`);
  }
  if (data !== true) {
    throw new Error("FREE_DISCOVERY_FINALIZE_FAILED:claim_not_found");
  }
}

export async function releaseFreeDiscoverySession(
  userId: string,
  claim: FreeDiscoverySessionClaim | null,
) {
  if (!claim) return;
  ...
  try {
    ...
    const { error } = await admin.rpc(
      "leadbase_release_free_discovery_session",
      {
        p_user_id: userId,
        p_claim_token: claim.token,
      },
    ...
    if (error) {
      console.error("Could not release Free discovery claim:", error);
    }
  } catch (error) {
    console.error("Could not release Free discovery claim:", error);
  }
...
export function isFreeDiscoverySessionError(error: unknown) {
  return (
    error instanceof Error &&
    (error.message === "FREE_DISCOVERY_ALREADY_USED" ||
      error.message === "FREE_DISCOVERY_IN_PROGRESS")
  );
}
