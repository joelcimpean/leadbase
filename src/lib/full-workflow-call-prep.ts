...
export async function generateAndSaveWorkflowCallPrep(input: {
  ...
  const visualForAi = visualAnalysisForPolicy(
    aiContextPolicy,
    lead.visual_analysis,
  );

  const websiteContext = [
    aiContextPolicy.allowStructuralFindings && lead.website_score !== null
      ? `Website score: ${lead.website_score}`
      : null,
    aiContextPolicy.allowStructuralFindings && lead.opportunity_score !== null
      ? `Opportunity score: ${lead.opportunity_score}`
      : null,
    aiContextPolicy.planId !== "free" && lead.visual_score !== null
      ? `Visual score: ${lead.visual_score}`
      : null,
    aiContextPolicy.planId !== "free" && lead.redesign_potential !== null
      ? `Redesign potential: ${lead.redesign_potential}`
      : null,
    aiContextPolicy.allowStructuralFindings
      ? safeJson(lead.website_findings)
      : null,
    safeJson(visualForAi),
  ].filter(Boolean).join("\n\n");
  ...
  try {
    const generated = await generateCallPrep({
      ...
      priority: aiContextPolicy.allowStructuralFindings ? lead.priority : null,
      notes: aiContextPolicy.allowUserNotes ? lead.notes : null,
      researchSummary: aiContextPolicy.allowResearchSummary ? lead.research_summary : null,
      websiteContext: websiteContext || null,
      outreachContext,
      conversationContext: aiContextPolicy.allowEmailThread ? conversationContext : null,
      latestReplyContext: aiContextPolicy.allowEmailThread ? latestReplyContext : null,
      previewContext: aiContextPolicy.allowPreviewSignals ? previewContext : null,
    });
